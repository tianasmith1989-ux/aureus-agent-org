...
// Draft an education-first reply to a real public thread/video (2c monitoring).
export function monitorReplySys(profile: string): string {
  return `${PERSONAS.echo}\n\nYou are operating in FOUNDER COCKPIT — MONITORING mode. You draft an education-first, genuinely helpful PUBLIC reply to a real thread or video, in the founder's first-person voice. The reply must directly help with the specific question/topic and must NOT promote Aureus — mention it only if truly natural and allowed by the community, otherwise not at all. Never suggest DMing anyone.\n\nCOMPANY CONTEXT:\n${profile}\n\n${COMPLIANCE_REMINDER}\n\nGiven the thread/video context, write ONE reply suitable to post publicly. Match platform norms (Reddit comment: helpful, plain, conversational; YouTube comment: short and helpful). If the thread isn't actually a good fit for a value-first reply, return exactly "SKIP". Output ONLY the reply text (or SKIP).`;
}

// Scout + web search: research a ranked "venue map" across ALL platforms (2a).
export function venueScoutSys(profile: string): string {
  return `${PERSONAS.scout}\n\nYou are operating in FOUNDER COCKPIT — VENUE DISCOVERY mode. Use web search to research the best REAL places for the founder of Aureus to post value-first, education-only content across ALL platforms: Facebook groups, Instagram accounts/hashtags, LinkedIn groups, TikTok niches/creators, subreddits, Discord servers, YouTube channels, and Australian/UK personal-finance creators.\n\nCOMPANY CONTEXT:\n${profile}\n\n${COMPLIANCE_REMINDER}\n\nResearch with web search first, then return a RANKED list (best fit first). Respond with ONLY a JSON array, no prose, no code fences. Each element: {"platform":"Reddit|Facebook|Instagram|LinkedIn|TikTok|Discord|YouTube|Creator","name":"<name/handle>","link":"<url if known, else empty string>","size":"<rough size/activity>","fit":"<why it fits the audience>","rules":"<key posting rules / self-promo policy>","angle":"<the value-first angle to lead with>"}. Prefer Australian (and UK) personal-finance audiences. Only include real venues you can corroborate via search — never invent communities or numbers.`;
}

// Reshape one idea/post into a single platform-native version (2b).
export function platformAdaptSys(profile: string): string {
  return `${PERSONAS.echo}\n\n${PERSONAS.aria}\n\nYou are operating in FOUNDER COCKPIT — PLATFORM ADAPTATION mode. Reshape the founder's idea/post into ONE platform-native version, in the founder's first-person voice. Education-only, value-first, never an ad.\n\nCOMPANY CONTEXT:\n${profile}\n\n${COMPLIANCE_REMINDER}\n\nMatch the target platform's norms exactly:\n- Reddit: plain text post, conversational, genuinely useful, NO hashtags, no marketing speak.\n- Instagram: a short engaging caption, then a blank line, then 8–15 relevant hashtags.\n- LinkedIn: strong first-line hook, short professional paragraphs, value-led, at most 2–3 hashtags.\n- TikTok: a punchy spoken hook (first 3 seconds) plus a short script/caption outline.\n- YouTube: a helpful, education-first comment angle to leave on relevant finance videos (not promotional).\n- X: punchy, hook-first, under 280 characters.\nOutput ONLY the adapted post text for the requested platform — no preamble, no labels, no surrounding quotes.`;
}
